...
const SupervisorDashboard = () => {
  ...
  useEffect(() => {
    ...
    const unsubIssues = onSnapshot(qIssues, (snapshot) => {
      ...
      let active = myZoneIssues.filter(i => i.status !== "Resolved");
  ...
  const handleAddWorker = async (e) => {
    ...
    try {
      ...
      if (existingWorker) {
        if (existingWorker.isDeleted) {
          await updateDoc(doc(db, "workers", existingWorker.id), { 
            isDeleted: false, name: newWorkerName, type: newWorkerType 
          });
        ...
        await setDoc(doc(db, "workers", workerUid), {
          name: newWorkerName, email: newWorkerEmail, type: newWorkerType, 
          supervisorId: auth.currentUser.uid, isDeleted: false
        });
      ...
      setNewWorkerName(''); setNewWorkerEmail(''); setNewWorkerPassword('');
    } catch (error) {
            ...
            <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
              <div className="card" style={{ flex: '1', minWidth: '300px', padding: '24px' }}>
                ...
                <form onSubmit={handleAddWorker} style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginTop: '15px' }}>
                  <div><label style={{ fontWeight: 'bold', fontSize: '0.9rem', color: '#64748b' }}>Worker Full Name</label><input type="text" required value={newWorkerName} onChange={(e) => setNewWorkerName(e.target.value)} style={{ width: '100%', padding: '10px', marginTop: '5px', borderRadius: '5px', border: '1px solid #cbd5e1' }} /></div>
                  <div><label style={{ fontWeight: 'bold', fontSize: '0.9rem', color: '#64748b' }}>Department</label>
                  ...
                  <div><label style={{ fontWeight: 'bold', fontSize: '0.9rem', color: '#64748b' }}>App Login Email</label><input type="email" required value={newWorkerEmail} onChange={(e) => setNewWorkerEmail(e.target.value)} style={{ width: '100%', padding: '10px', marginTop: '5px', borderRadius: '5px', border: '1px solid #cbd5e1' }} /></div>
                  <div><label style={{ fontWeight: 'bold', fontSize: '0.9rem', color: '#64748b' }}>App Password</label>
                    <div style={{ position: 'relative', display: 'flex', alignItems: 'center', marginTop: '5px' }}>
                      <input type={showPassword ? "text" : "password"} required minLength="6" value={newWorkerPassword} onChange={(e) => setNewWorkerPassword(e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: '5px', border: '1px solid #cbd5e1', paddingRight: '60px' }} />
                      <button type="button" onClick={() => setShowPassword(!showPassword)} style={{ position: 'absolute', right: '10px', background: 'none', border: 'none', color: '#3b82f6', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.85rem' }}>{showPassword ? "HIDE" : "SHOW"}</button>
                    </div>
                  </div>
                  <button type="submit" disabled={isAdding} style={{ backgroundColor: '#10b981', color: 'white', padding: '12px', border: 'none', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold', marginTop: '10px' }}>{isAdding ? 'Generating...' : 'Create Worker App Login'}</button>
                </form>
              </div>
                  ...
                  <table>
                    ...
                    <tbody>
                      {activeWorkers.map(worker => {
                        ...
                        return (
                        <tr key={worker.id}>
                          <td><strong>{worker.name}</strong><br/><span style={{fontSize: '0.8rem', color: '#64748b'}}>{worker.email}</span></td>
                          ...
                          <td>
                            <button onClick={() => handleRemoveWorker(worker.id, worker.name)} disabled={isBusy} style={{ background: isBusy ? '#f1f5f9' : '#fee2e2', color: isBusy ? '#94a3b8' : '#ef4444', border: 'none', padding: '6px 12px', cursor: isBusy ? 'not-allowed' : 'pointer', fontWeight: 'bold' }}>{isBusy ? '🔒 Busy' : 'Archive'}</button>
                          </td>
                        </tr>
                      )})}
                    </tbody>
          ...
          {view === 'archive' && (
            <div className="card">
              ...
              <div className="table-responsive">
                <table>
                  <thead>
                    <tr><th>Issue ID</th><th>Type / Location</th><th>Proof of Work</th><th>Resolved By</th><th>Action</th></tr>
                  </thead>
                  <tbody>
